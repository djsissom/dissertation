...
def main():
	...
	for x_col, xlabel in zip(x_cols, xlabels):
		make_plot(dc, x_col, halos, header, xlabel, use_log=False)
	for x_col, xlabel in zip(x_log_cols, xlabels_log):
		make_plot(dc, x_col, halos, header, xlabel, use_log=True)
...
ylabel = r"$\mathrm{(c_{2LPT} - c_{ZA}) / c_{avg}}$"

#c_source = 'density_profile'
c_source = 'rockstar'

plot_base = 'plots/diff_c_-_vs_-_'
plot_ext  = '.eps'
